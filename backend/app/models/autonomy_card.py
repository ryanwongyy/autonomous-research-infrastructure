...
class AutonomyCard(Base):
    ...
    role_autonomy_json: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # JSON: {"scout": "full_auto"/"supervised"/"human_driven", ...}
    human_intervention_points_json: Mapped[str | None] = mapped_column(
        Text
    )  # JSON array of {role, stage, description}
