...
class ClaimMap(Base):
    ...
    )  # "empirical", "doctrinal", "historical", "theoretical", "descriptive"
    source_card_id: Mapped[str | None] = mapped_column(
        String(64), ForeignKey("source_cards.id"), index=True
    )  # NULL if linked to result object
    source_snapshot_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("source_snapshots.id"), index=True
    )
    source_span_ref: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: {document_id, page, paragraph, span_start, span_end}
    result_object_ref: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: {analysis_run_id, table, column, row}
