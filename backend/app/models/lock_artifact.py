...
class LockArtifact(Base):
    ...
    family_id: Mapped[str] = mapped_column(
        String(8), ForeignKey("paper_families.id"), nullable=False, index=True
    )
    lock_protocol_type: Mapped[str] = mapped_column(
        String(64), nullable=False
    )  # must match family's protocol
    version: Mapped[int] = mapped_column(
        Integer, default=1
    )  # major version; new version on substantive change
    ...
    source_manifest_hash: Mapped[str | None] = mapped_column(
        String(64)
    )  # hash of source manifest at lock time
    immutable_fields: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # JSON array of field names that cannot change
    ...
    locked_by: Mapped[str | None] = mapped_column(
        String(128)
    )  # human who approved lock, or "system"
    superseded_by: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("lock_artifacts.id"), index=True
    )  # newer version
