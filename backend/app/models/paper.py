...
class Paper(Base):
    ...
    )  # draft, locked, analyzing, drafting, reviewing, revision, candidate, submitted, public, rejected, killed
    review_status: Mapped[str] = mapped_column(
        String(32), default="awaiting"
    )  # awaiting, peer_reviewed, issues, errors
    ...
    lock_hash: Mapped[str | None] = mapped_column(
        String(64)
    )  # SHA-256 of active lock artifact
    lock_version: Mapped[int] = mapped_column(
        Integer, default=0
    )  # current lock version (0 = not yet locked)
    lock_timestamp: Mapped[datetime | None] = mapped_column(
        DateTime
    )  # when design was locked
    release_status: Mapped[str] = mapped_column(
        String(32), default="internal", index=True
    )  # internal, candidate, submitted, public
    funnel_stage: Mapped[str] = mapped_column(
        String(32), default="idea", index=True
    )  # idea, screened, locked, ingesting, analyzing, drafting, reviewing, revision, benchmark, candidate, submitted, public, killed
    secondary_family_id: Mapped[str | None] = mapped_column(
        String(8), ForeignKey("paper_families.id"), index=True
    ...
    overall_screening_score: Mapped[float | None] = mapped_column(
        Float
    )  # weighted composite
    kill_reason: Mapped[str | None] = mapped_column(Text)  # why this project was killed
    data_question_cluster: Mapped[str | None] = mapped_column(
        String(128)
    ...
    family: Mapped["PaperFamily | None"] = relationship(
        back_populates="papers", foreign_keys=[family_id], lazy="joined"
    )
    secondary_family: Mapped["PaperFamily | None"] = relationship(
        foreign_keys=[secondary_family_id], lazy="joined"
    )
    lock_artifacts: Mapped[list["LockArtifact"]] = relationship(
        back_populates="paper", lazy="selectin"
    )
    claim_maps: Mapped[list["ClaimMap"]] = relationship(
        back_populates="paper", lazy="selectin"
    )
