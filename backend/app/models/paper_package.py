...
class PaperPackage(Base):
    ...
    )  # SHA-256 Merkle root of all artifacts
    ...
    authorship_declaration: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: human authors and contributions
    ai_contribution_log: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: Claude's role at each pipeline stage
