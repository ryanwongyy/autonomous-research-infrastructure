...
class SourceCard(Base):
    ...
    )  # e.g. "government_registry"
    temporal_coverage: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: {start_date, end_date, notes}
    ...
    claim_permissions: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # JSON array: what source CAN support
    claim_prohibitions: Mapped[str] = mapped_column(
        Text, nullable=False
    )  # JSON array: what source CANNOT support
    required_corroboration: Mapped[str | None] = mapped_column(
        Text
    )  # JSON: when corroboration is needed
    ...
    fragility_score: Mapped[float] = mapped_column(
        Float, default=0.0
    )  # 0.0 (robust) to 1.0 (brittle)
    ...
    snapshots: Mapped[list[SourceSnapshot]] = relationship(
        back_populates="source_card", lazy="selectin"
    )
