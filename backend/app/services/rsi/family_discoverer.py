...
def _cluster_papers_by_keywords(
    ...
    for p in papers:
        combined_text = (
            f"{p.get('kill_reason', '')} {p.get('idea_card_yaml', '')} {p.get('method', '')}"
        )
        ...
        results.append(
            {
                ...
                "common_kill_reasons": [reason for reason, _ in kill_reasons.most_common(5)],
                "common_methods": [method for method, _ in methods.most_common(5)],
...
async def approve_family_proposal(
    ...
    """
    result = await session.execute(select(FamilyProposal).where(FamilyProposal.id == proposal_id))
    ...
    if proposal.status != "proposed":
        raise ValueError(
            f"Proposal {proposal_id} is not in 'proposed' status (current: '{proposal.status}')"
        )
    ...
    existing = await session.execute(select(PaperFamily.id).where(PaperFamily.id == new_family_id))
...
async def get_family_proposals(
    ...
    query = select(FamilyProposal).order_by(FamilyProposal.created_at.desc())
    ...
    return [
        {
            ...
            "source_cluster": (safe_json_loads(p.source_cluster_json, None)),
            "kill_reasons": (safe_json_loads(p.kill_reasons_json, None)),
